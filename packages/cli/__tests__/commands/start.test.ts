...
describe("start command — orchestrator session strategy display", () => {
  ...
  it("picks the most recent across live and restorable orchestrators", async () => {
    mockConfigRef.current = makeConfig({ "my-app": makeProject() });

    const now = Date.now();
    // Live -2 (older) vs restorable -3 (newer) — the newer one wins regardless
    ...
    const output = getLoggedOutput();
    expect(output).toContain("/sessions/app-orchestrator-3");
  });
